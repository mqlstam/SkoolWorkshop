...
<template>
  <div class="row box-header">
    ...
    <div class="col d-flex align-items-center">
      <h3 class="m-0"> Add products to workshop</h3>
    </div>
  </div>
  ...
  <div class="row box bg-white border-top">
    ...
    <div v-for="product in filteredProducts" :key="product.id"
      ...
      <div class="ms-auto">
        ...
        <button v-if="!productsInWorkshop.includes(product)" class="btn btn-sm p-1 hover-darken"
          @click="handleStockClick(product)">
          <font-awesome-icon :icon="['fas', 'plus']" class="fa-1x m-2"
            style="width:20px;height:20px;"></font-awesome-icon>
        </button>
        <font-awesome-icon v-else :icon="['fas', 'check']" class="fa-1x m-2 text-success"
          style="width:20px;height:20px;"></font-awesome-icon>
      </div>
    </div>
  </div>

  <div v-if="!filteredProducts.length" class="d-flex align-items-center my-5 justify-content-center">
      <p class="text-danger">All products are already associated with this workshop.</p>
    </div>
</template>
